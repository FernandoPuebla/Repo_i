...
public class BarajaEspaniola extends Baraja
{
    ...
    public void crearBaraja() {
        ...
        this.cartas = (Carta<PalosBarajaEspaniola>[]) new Carta[numCartas];
        ...
        for (int i = 0; i < palos.length; i++) {
            ...
            //Recorro los numeros
            for (int j = 0; j < cartasPorPalo; j++) { if (incluye_8_9) { cartas[((i * cartasPorPalo) + j)] = new Carta(j + 1, palos[i]); } else { //Las posiciones del 8 y del 9 son el 7 y el 8 (emepzamos en 8)
                if (j >= 7) {
